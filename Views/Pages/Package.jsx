...
const Package = () => {
  ...
  useEffect(() => {
    const fetchPackageData = async () => {
      try {
        const response = await fetch(`${process.env.VITE_API_ORIGIN}/api/packages/${packageName}`);
  ...
  useEffect(() => {
    const fetchExtraData = async () => {
      try {
        const response = await fetch(`${process.env.VITE_API_ORIGIN}/api/extras/${packageName}`);
          ...
          <div className="packages-container">
            <div className={`pkg-info-container ${packageData.slug}`}>
              <div className="pkg-info-left-content">
                <div>
                  <h1 className="pkg-info-header">{packageData.title}: {packageData.tagline}</h1>
                  <h2 className="pkg-info-header-desc">{packageData.subtitle}</h2>
                  <div className="prices-container">
                    <h3 className={`text-${packageName}`}>${packageData.price}</h3>
                    <p className="old-price">${packageData.oldPrice}</p>
                  </div>
                  <div className="pkg-info-list">
                    <ul>
                      {packageData.features.slice(0, Math.ceil(packageData.features.length / 2)).map((feature, index) => (
                        <li key={index}>
                          <Icon
                            icon={feature.includeType === "included" ? "faCircleCheck" : 
                              feature.includeType === "vip" ? "faCrown" : 
                              "faCircleXmark"}
                            type="solid"
                            color={ feature.includeType === "included" ? "#45a834" : 
                              feature.includeType === "vip" ? "#f7ef8a" : 
                              "#dd6666"}
                              bgColor={
                                feature.includeType === "vip" ? "#000010" :  undefined
                              }
                          />
                          {feature.name}
                        </li>
                    ...
                    <ul>
                      {packageData.features.slice(Math.ceil(packageData.features.length / 2)).map((feature, index) => (
                        <li key={index}>
                          <Icon
                            icon={feature.includeType === "included" ? "faCircleCheck" : "faCircleXmark"}
                            type="solid"
                            color={feature.includeType === "included" ? "#45a834" : "#ff3b3b"}
                          />
                          {feature.name}
                        </li>
                      ))}
              ...
              <div className="pkg-info-right-content">
                ...
                <div>
                  <div className="action-buttons">
                    <button onClick={() => handleOrderClick()} className={`bg-${packageName}`}>
                      Order Now
                    </button>
